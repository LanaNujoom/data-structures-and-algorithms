...
const listFoods = (recipe) => {
    ...
    for (let index = 0; index < recipe.ingredients.length; index++) {
        let superTemp = recipe.ingredients[index].slice(recipe.ingredients[index].indexOf(' ') + 1, recipe.ingredients[index].length);
        result.push(superTemp.slice(superTemp.indexOf(' ') + 1, superTemp.length));
    }
    return result;
};
